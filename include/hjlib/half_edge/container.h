...
bool __crazy_vc_iter_equal(const ITR &a, const ITR &b)
{
  return !memcmp(&a, &b, sizeof(ITR));
}

//! @brief return itr == null, makes itr look like a pointer
template <typename TMPL>
bool operator!(const iterator_tmpl<TMPL> &itr) { 
#if defined(_MSC_VER) && _HAS_ITERATOR_DEBUGGING != 0
  return __crazy_vc_iter_equal<TMPL>(itr(), TMPL());
#else
  return itr() == TMPL();
#endif
}

#define __CRAZY_VC_BOOLEAN(type)                                        \
  bool operator == (const type &other) const { return __crazy_vc_iter_equal<type>(*this, other); }
...
#if defined(_MSC_VER) && _HAS_ITERATOR_DEBUGGING != 0
    __CRAZY_VC_BOOLEAN(const_iterator)
#endif
  };
 ...
 public:
  ...
  class iterator_base : public std::iterator<std::bidirectional_iterator_tag, value_type>
  {
   public:
    ...
    template <typename O>
    bool operator < (const iterator_base<O> &i) const  { return addr_ < i.addr_; }

    template <typename O>
    bool operator <= (const iterator_base<O> &i) const  {
      return (addr_ < i.addr_) || (addr_ == i.addr_);
    }
    ...
    bool is_deleted() const {return this_->std_vec_.is_deleted(addr_);}
   protected:
